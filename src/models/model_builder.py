...
class Bert(nn.Module):
    def __init__(self, temp_dir, load_pretrained_bert, bert_config):
        print(f"temp_dir: {temp_dir}, load_pretrained_bert: {load_pretrained_bert}, bert_config: {bert_config}")
        super(Bert, self).__init__()
        if(load_pretrained_bert):
            self.model = BertModel.from_pretrained('bert-base-uncased', cache_dir=temp_dir)
            print('Pre-trained BERT loaded successfully!')
        else:
            self.model = BertModel(bert_config)
            print('BERT initialized without pre_trained weights!')
