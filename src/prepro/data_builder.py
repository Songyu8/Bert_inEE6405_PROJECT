...
class BertData():
    ...
    def preprocess(self, src, tgt, oracle_ids):
        ...
        original_src_txt = [' '.join(s) for s in src]   #生成原始文本的列表，遍历src中的句子，用空格连接起来，将句子转化为字符串
        ...
        src_txt = [' '.join(sent) for sent in src]    #将src中的句子转化为字符串格式，用空格连接
        # text = [' '.join(ex['src_txt'][i].split()[:self.args.max_src_ntokens]) for i in idxs]
        # text = [_clean(t) for t in text]
        text = ' [SEP] [CLS] '.join(src_txt)  #这是对句子的分割
        src_subtokens = self.tokenizer.tokenize(text)  #调用tokenizer对text进行处理
        src_subtokens = src_subtokens[:510]     #切片，确保输入不会超过模型限制
        src_subtokens = ['[CLS]'] + src_subtokens + ['[SEP]']

        src_subtoken_idxs = self.tokenizer.convert_tokens_to_ids(src_subtokens)  #分词后的子词转换为对应的词汇表中的索引
